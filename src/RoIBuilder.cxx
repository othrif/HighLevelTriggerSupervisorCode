...
const uint32_t minBacklog=9500;
	  ...
	  if(m_active_chan.find(rolId) != m_active_chan.end())
		{ 
		  ...
		  if(ev->count()==m_active_chan.size()) {
			...
			//elapsed=std::chrono::duration_cast<std::chrono::microseconds>(thistime-ev->m_start);

			//			if(TimeMe)
			//			  ERS_LOG(" Receiving all fragments took " <<  elapsed.count() << " us");
...
bool RoIBuilder::getNext(uint32_t & l1id,uint32_t & count,uint32_t * & roi_data,uint32_t  & length)
{
  ...
  // will have to add code to handle the stop
  m_done.pop(ev);
  l1id=ev->data()[5];
  count=ev->count();
  linkList=ev->links();
  ...
  if(count == m_nactive ){
	...
	// Some printouts to investigate the code
	if(DebugMe){
	  std::string linksin;
	  for (uint32_t i=0;i<linkList.size();i++)
		linksin+=
		  static_cast<std::ostringstream*>( &(std::ostringstream() << linkList.at(i)))->str()+" ";
	  ERS_LOG("links:"<<linksin.c_str()<<"reported as complete for lvl1id:" << l1id); 
	  if( DebugData){
		ERS_LOG(" data dump:");
