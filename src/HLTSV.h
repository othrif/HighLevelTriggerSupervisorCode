...
protected:
	...
	out << Rate << Recent_LVL1_ID << Recent_Global_ID << Busy << LVL1DecodingErrors << RoIB_Bandwidth;
	out << RoIB_Pending_RNP << RoIB_Pending_DAQ << RoIB_time_Build << RoIB_time_Process;
	out << RoIB_timeout << RNP_Free_pages << RNP_Used_pages << RNP_Most_Recent_ID << RNP_XOFF_state;
	out << RNP_XOFF_per << RNP_XOFF_count << RNP_Down_stat << RNP_bufferFull << RNP_numberOfLdowns;
    }

    void refreshGuts( ISistream & in ){
	in >> LVL1Events >> AssignedEvents >> ReassignedEvents >> ProcessedEvents >> AvailableCores;
	in >> Rate >> Recent_LVL1_ID >> Recent_Global_ID >> Busy >> LVL1DecodingErrors >> RoIB_Bandwidth;
	in >> RoIB_Pending_RNP >> RoIB_Pending_DAQ >> RoIB_time_Build >> RoIB_time_Process;
	in >> RoIB_timeout >> RNP_Free_pages >> RNP_Used_pages >> RNP_Most_Recent_ID >> RNP_XOFF_state;
	in >> RNP_XOFF_per >> RNP_XOFF_count >> RNP_Down_stat >> RNP_bufferFull >> RNP_numberOfLdowns;
    }
